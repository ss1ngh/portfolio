...
export function ReadsContent() {
  return (
    <div className="relative flex flex-col justify-start min-h-dvh w-[1152px] px-6 pt-20 pb-24 items-center selection:bg-[#3235F8] selection:text-white">

      {/* label */}
      <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-16 text-[15rem] font-black lowercase tracking-tight text-[#0A0A0A]/5 select-none pointer-events-none whitespace-nowrap">
        reading corner
      </span>

      <section className="w-full max-w-[800px] flex flex-col items-center px-4">

        <div className="w-full flex flex-col gap-y-4 mb-18 items-center justify-center cursor-default">
          ...
          {DATA.reads.map((read) => (
            ...
            >
              ...
              <ArrowUpRightIcon
                size={18}
                weight="bold"
                className="text-[#A3A3A3] group-hover:text-[#3235F8] transition-all duration-200 group-hover:translate-x-0.5 group-hover:-translate-y-0.5"
              />
            </a>
          ))}
        </div>
